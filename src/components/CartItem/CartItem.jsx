            ...
            <div className="rounded-lg">
                {
                    productData.map((item, i) =>
                        ...
                        <div key={i} className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                            <img src={item.image} alt="product-image" className="w-full rounded-lg sm:w-20" />
                            <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                                ...
                                <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div onClick={() => dispatch(deleteItem(item._id)) & toast.error(`${item.title} is removed from cart`)}>
                ...
                <button onClick={()=> dispatch(resetCart()) & toast.error('Your card is empty')} className="btn btn-secondary">reset cart</button>
            </div>
        </div>
    );
};
